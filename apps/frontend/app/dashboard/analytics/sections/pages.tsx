...
const WB_SECTIONS: Record<string, string> = {
  orders:'Заказы', feedbacks:'Отзывы', reviews:'Отзывы', questions:'Вопросы', products:'Товары',
  prices:'Цены', stocks:'Остатки', remains:'Остатки', supplies:'Поставки', supply:'Поставки',
  advertising:'Реклама', analytics:'Аналитика', finance:'Финансы',
  chat:'Чат', promotions:'Акции', promotion:'Акции', knowledge:'База знаний', other:'Прочее',
};
                        ...
                        <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
                          {actions.map(([action, count]) => (
                            <div key={action} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                              <span style={{ fontSize:'12px', color:'var(--text-secondary)' }}>{ACTION_LABELS[action] ?? action.replace(/^(wb_|ozon_)/, '').replace(/_/g, ' ')}</span>
